...
# Note: All whitespace is removed from the token
...
BEEHIIV_FEED_URL = re.sub(
    r'\s+', '', os.getenv(
        'BEEHIIV_FEED_URL', 'https://nm2t-newsletter.beehiiv.com/archive'))
    ...
    try:
        ...
        if not feed.entries:
            await update.message.reply_text(
                ...
                "The feed may not be configured yet. "
...
def main() -> None:
    ...
    try:
        ...
        logger.info(
            f"Initializing bot with token: "
            f"{BOT_TOKEN[:10]}...{BOT_TOKEN[-10:]}"
        )
